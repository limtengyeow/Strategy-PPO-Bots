    ...
    for i in range(len(date_ranges) - 1):
        ...
        while True:
            try:
                ...
                print(
                    f"    ❌ Error fetching chunk {chunk_from} to {chunk_to} for {ticker}: {e}"
                )
                results = []
                break
...
def add_features(df, features, prefix):
    ...
    for f in features:
        ...
        if f["field"] == "ema":
            ...
            feature_series = (df["close"] * df["volume"]).cumsum() / df[
                "volume"
            ].cumsum()
        ...
        if f.get("normalize"):
            ...
            if method == "zscore":
                df[col] = (
                    feature_series - feature_series.mean()
                ) / feature_series.std()
            elif method == "rolling_zscore":
                df[col] = (
                    feature_series - feature_series.rolling(f["window"]).mean()
                ) / feature_series.rolling(f["window"]).std()
        else:
...
def generate_simulated_data(scenario, start_date, end_date, cfg):
    ...
    prices = sim_cfg.get("base_price", 150) + np.cumsum(
        np.random.normal(0, sim_cfg.get("price_volatility", 0.5), len(dates))
    )
    volume = np.random.normal(
        sim_cfg.get("volume_mean", 500000),
        sim_cfg.get("volume_std", 100000),
        len(dates),
    )
    return pd.DataFrame(
        {
...
def merge_dataframes(base_df, higher_df):
    ...
    merged = pd.merge(
        base_df, higher_df.drop(columns=["timestamp"]), on="date", how="left"
    )
...
for ticker in TICKERS:
    ...
    if ticker.startswith("@SIM"):
        ...
        try:
            ...
            print(
                f"❌ Error in simulation config: Missing key {e}. Skipping simulation."
            )
        continue

    all_configured_prefixes = [
        k.replace("FEATURES_", "").lower()
        for k in cfg.get("features", {})
    ...
    print(
        f"Found {len(precompute_prefixes)} non-intraday timeframe(s) to pre-compute: {precompute_prefixes}"
    )
    print(
        f"Found {len(main_prefixes)} intraday timeframe(s) for main processing: {main_prefixes}"
    )
    ...
    if precompute_prefixes:
        ...
        for prefix in precompute_prefixes:
            ...
            calendar_days_back = convert_trading_days_to_calendar_days(
                trading_days_buffer
            )
            start = (
                datetime.strptime(START_DATE, "%Y-%m-%d")
                - timedelta(days=calendar_days_back)
            ).strftime("%Y-%m-%d")
            ...
            if raw_df is not None and not raw_df.empty:
                ...
                filepath = os.path.join(
                    DATA_DIR, f"{ticker}_{prefix}_precomputed.parquet"
                )
    ...
    main_features_config = {
        f"FEATURES_{p.upper()}": cfg["features"][f"FEATURES_{p.upper()}"]
        for p in main_prefixes
    }
    main_all_features = [
        feat for features in main_features_config.values() for feat in features
    ]
    main_trading_days_buffer = compute_buffer_bars(
        main_all_features, apply_obs_window=True
    )
    main_calendar_days_back = convert_trading_days_to_calendar_days(
        main_trading_days_buffer
    )
    ...
    for prefix in main_prefixes:
        ...
        start = (
            datetime.strptime(START_DATE, "%Y-%m-%d")
            - timedelta(days=main_calendar_days_back)
        ).strftime("%Y-%m-%d")
        ...
        if raw_df is not None and not raw_df.empty:
            ...
            df_by_prefix[prefix] = pd.merge(
                raw_df, feature_df, on="timestamp", how="left"
            )
        else:
    ...
    if df_by_prefix:
        ...
        base_prefix = min(
            main_prefixes, key=lambda x: timeframe_order.get(x, float("inf"))
        )
        ...
        for prefix in main_prefixes:
            if prefix != base_prefix:
                ...
                feature_cols = [
                    c for c in df_by_prefix[prefix].columns if c.startswith(prefix)
                ]

                left_df = merged.sort_values("timestamp")
                right_df = (
                    df_by_prefix[prefix][["timestamp"] + feature_cols]
        ...
        if output_cols:
            ...
            if missing_cols:
                print(
                    f"  ⚠️ These requested output columns were not found and have been ignored: {list(missing_cols)}"
                )
